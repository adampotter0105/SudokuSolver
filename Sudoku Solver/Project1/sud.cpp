	...
	for (int i = 0; i < 9; i++) {
		for (int j = 0; j < 9; j++) {
			...
			//block
				...
				if (inSudoku[i][j] != 0) { for (int k = 0; k < 9; k++) { workSud[i][j][k] = 0; } LtoB(); }
		}
	}
}
						...
						if (equal(workSud[i][j], workSud[a][b]) && inSudoku[a][b] == 0 && count(workSud[i][j]) == 2 && count(workSud[a][b]) == 2 && (i != a || j != b)) {
							...
							if (i == a) {
								for (int y = 0; y < 9; y++) {
									if (y != b && y != j) { 
										for (int z = 0; z < 9; z++) {
											if (workSud[i][y][z] == workSud[i][j][z]) { workSud[i][y][z] = 0; 
											}
										}
									}
							...
							if (j == b) {
								for (int y = 0; y < 9; y++) {
									if (y != a && y != i) {
										for (int z = 0; z < 9; z++) {
											if (workSud[y][j][z] == workSud[i][j][z]) { workSud[y][j][z] = 0; 
											}
										}
									}
								}
							...
							if (convert(i, j, 0) == convert(a, b, 0)) {
								...
								for (int y = 0; y < 9; y++) {
									if (y != convert(i,j,1) && y != convert(a,b,1)) {
										for (int z = 0; z < 9; z++) {
											if (workBlock[convert(i, j, 0)][y][z] == workSud[i][j][z]) {
												
												workBlock[convert(i, j, 0)][y][z] = 0;
											}
